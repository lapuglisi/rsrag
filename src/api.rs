...
use crate::engine::{
    llama::{
        LLAMA_DEFAULT_NPREDICT, LLAMA_DEFAULT_STREAM, LLAMA_DEFAULT_TEMPERATURE,
        LLAMA_DEFAULT_TOP_K, LLAMA_DEFAULT_TOP_P, LLAMA_RERANK_DEFAULT_THRESHOLD,
        LlamaCompletionMessage, LlamaCompletionRequest, LlamaEngine, RerankRequest,
    },
    qdrant::{
        QDRANT_QUERY_DEFAULT_LIMIT, QDRANT_QUERY_DEFAULT_THRESHOLD, QdrantEngine, QdrantQuery,
    },
};
...
impl<'r> RagApi<'r> {
    ...
    pub async fn listen(self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let app: Router = Router::new()
            ...
            .route("/models", get(api_get_models))
            .with_state(st)
            .layer(cors_layer);
...
#[axum::debug_handler]
async fn api_get_models(State(st): State<RagApiState>) -> impl IntoResponse {
    st.llama.get_models().await.into_response()
}
